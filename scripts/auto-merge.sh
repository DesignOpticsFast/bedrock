...
# Auto-merge and cleanup script for Bedrock repository
...
if [ "$STATUS_CHECKS" -gt 0 ]; then
    echo "❌ Not all status checks are passing. Please wait for checks to complete."
    exit 1
fi

echo "✅ All status checks are passing"
